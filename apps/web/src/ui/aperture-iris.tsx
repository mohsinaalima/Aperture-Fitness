...
export const ApertureIrisProgress: React.FC<ApertureIrisProps> = ({
  ...
  // 0% value = fully open aperture (large hole), 100% = fully closed/tight iris
  ...
  return (
    <svg
      ...
      className={cn("transition-transform duration-320 ease-mechanical", className)}
      {...props}
    >
      {/* Outer Housing Ring */}
      <circle
          ...
          <g key={index} transform={`rotate(${currentAngle} 50 50)`}>
            <path
              d={`M 50 ${50 - innerRadius} 
                 ...
                 Z`}
              fill={normalizedValue > 0 ? "var(--color-accent-primary)" : "transparent"}
